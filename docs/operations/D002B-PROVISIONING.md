...
2. **Checkpoint B2 — Neon.** After B1 is recorded, create the Neon project/branch/database and separate `bbb_preview_deploy` and `bbb_preview_runtime` login roles with provider-generated credentials. The deploy role owns `public` and migration-created objects; the runtime role receives `CONNECT` only from the database administrator. Run migrations as deploy; the committed deployment script then revokes runtime schema creation and applies the exact SELECT/DML/sequence grants in `preview-runtime-grants.ts`. Select the nearest compatible Neon/Render regions, record them, enable only the approved Launch recovery window, and prove runtime DDL, trigger-disable, template/canonical writes, role grants, and migration attempts fail. Render receives only the TLS pooled runtime URL; the protected GitHub environment receives only the direct deploy-role URL and its exact non-pooler hostname as `PREVIEW_DATABASE_HOST`. Stop for review before Render if any privilege test fails.
