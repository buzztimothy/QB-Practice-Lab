...
| Concern | Decision |
|---|---|
| Authentication | Clerk, email/password, email verification, invite-only access mode; no social login or organizations |
...
Use one Neon project dedicated to BBB Preview, with one long-lived `preview` branch and one application database/role. Select the Neon and Render regions together to minimize latency and public egress. Do not share this project with local tests or future Production.
...
Render is selected as one paid web service because it runs the existing long-lived Node process directly, accepts explicit build/start commands, supports GitHub deployments, encrypted variables, HTTP readiness checks, logs, zero-downtime deploys, and artifact rollback. Configure the service to bind `0.0.0.0:$PORT`; D-002 may add the small `PORT` alias/graceful-shutdown adapter needed because the current server reads `WEB_PORT`.
...
The conceptual URL is `https://bbb-practice-preview.<controlled-domain>` (or the temporary Render subdomain during D-002). Browser, assets, authentication exchange, application API, and actions remain on that origin. Clerk redirects only to allowlisted Preview URLs. There is no CORS-enabled business API; cookies never need cross-origin credential mode. Exact `APP_ORIGIN`, `SameSite=Strict`, Secure cookies, and Origin validation remain mandatory.
...
| `CLERK_WEBHOOK_SECRET` | optional lifecycle webhook | Yes | Preview | webhook rejected; core login remains fail-closed |
...
Render encrypted environment settings hold runtime secrets. Deployment credentials belong only to the serialized deploy job, not the web service. GitHub secrets may hold a narrowly scoped Render deploy hook/token if CI promotes. Never commit real `.env`, database credentials, Clerk secrets, tokens, session values, or domain control credentials. Rotation creates a replacement credential, updates the secret store, rolls/restarts safely, verifies readiness, then revokes the old credential. Database and Clerk rotation require explicit session/connection impact review.
...
2. Acquire a deployment advisory lock in Preview PostgreSQL (or enforce one-at-a-time CI environment concurrency, with the database lock as defense in depth).
3. Verify the target database identity/project marker and current migration status.
4. Run committed `prisma migrate deploy` through `DIRECT_DATABASE_URL`; never `db push`. Failure stops promotion.
5. Build once, run canonical bootstrap with the same artifact, rerun it to prove idempotency, and verify installed version/digests without exposing manifest content.
...
Preview telemetry includes deployment/startup SHA, migration/bootstrap outcome, readiness state changes, categorized authentication/link/provisioning failures, database connectivity failures, unexpected application errors, and CAS/idempotency conflicts. Include request correlation ID and internal student/attempt opaque IDs only where necessary. Never log cookies, raw BBB/Clerk tokens, passwords, authorization headers, Clerk secrets, hidden instructor facts, canonical manifests, complete student payloads, or protected documents.
...
Before pilot, an operator must be able—through documented provider/database procedures, not an admin UI—to invite a tester, activate/deactivate the internal student, revoke Clerk and all BBB sessions, inspect health/deployment records, preserve attempts, and direct reset/retry through normal application behavior. A support contact and incident owner are required.
...
At current published pricing (review again in D-002), Clerk Hobby can cover a very small Preview and its invite-only feature is documented; use a paid tier if required for support, branding, session-policy, or production-use expectations. Neon Launch describes a typical intermittent small database around the low tens of dollars monthly and provides a seven-day restore window. Render requires a continuously suitable paid web-service instance for a pilot—do not use a sleeping free instance for acceptance. Overall cost is low tens of dollars per month before domains/email/support, with low-to-moderate operational complexity. Auth0's invitation customization and Fly's Machine configuration would be materially more operational work; Supabase Auth Pro would start around its platform minimum while duplicating database-platform concerns.
...
5. **Neon Preview** — create one dedicated project/branch/database, runtime and deploy roles, pooled/direct secrets, region and restore window. Validate TLS, least privilege, connection bounds, PITR rehearsal. Stop if tests/PRs can access it.
...
- Confirm whether Clerk's selected paid/free tier satisfies BBB support, branding, session lifetime, and pilot contractual expectations on provisioning day.
- Set the pilot retention/review duration and named incident/support owner.
- Decide whether lifecycle webhooks are required initially; login-time validation plus BBB deactivation remains authoritative either way.
