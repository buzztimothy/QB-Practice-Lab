...
BBB's cookie is host-only (no `Domain` attribute), `Secure`, `HttpOnly`, `SameSite=Strict`, `Path=/`, and bounded by the configured TTL. Only one canonical hostname is active: while using the Render hostname it is the exact `APP_ORIGIN`; custom-domain cutover updates Clerk redirects/authorized parties and BBB origin together, verifies the new host, then redirects or disables the old hostname before accepting mutations. Render's internal/default hostname is never implicitly added to the mutation allowlist.
...
Proposed deterministic names are `bbb-practice-lab-preview` (Clerk application), `bbb-practice-lab-preview` (Neon project), `preview` (Neon branch), `bbb_practice_preview` (database), `bbb_preview_runtime` and `bbb_preview_deploy` (roles), and `bbb-practice-lab-preview` (Render service). Provider uniqueness suffixes may be added without changing logical names.
