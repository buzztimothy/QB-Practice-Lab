...
export function deriveSuncoastAssessment(coaching: SuncoastCoachingAttempt): SuncoastAssessmentAttempt {
  ...
  for (const event of coaching.interaction.evidence.p002.attempt.auditTrail) {
    ...
    const verifiedCorrect = event.verifiedCorrect === true || event.action === 'TRANSACTION_CORRECTED' || event.action === 'ACCOUNT_CONSOLIDATED' || event.action === 'CONTROL_VERIFIED' || event.action === 'SELF_CORRECTION' || event.selfCorrected === true;
