...
export type OperationalAccountRole = 'BANK' | 'UNDEPOSITED_FUNDS' | 'ACCOUNTS_RECEIVABLE' | 'CREDIT_CARD';
export type AttemptStatus = 'ACTIVE' | 'SUBMITTED' | 'RESET';
export interface TemplateAccount { readonly id: string; readonly code: string; readonly name: string; readonly kind: AccountKind; readonly operationalRole?: OperationalAccountRole }
...
export interface AttemptAccount { readonly id: string; readonly attemptId: string; readonly sourceTemplateAccountId: string; readonly code: string; readonly name: string; readonly kind: AccountKind; readonly operationalRole?: OperationalAccountRole }
...
export interface JournalEntry { readonly id: string; readonly attemptId: string; readonly description: string; readonly occurredOn: string; readonly lines: readonly JournalLine[]; readonly source?: { readonly kind: string; readonly id: string } }
export interface AttemptAction { readonly sequence: number; readonly kind: 'ATTEMPT_CREATED' | 'JOURNAL_POSTED' | 'DOCUMENT_UNLOCKED' | 'ATTEMPT_RESET' | 'CUSTOMER_CREATED' | 'INVOICE_POSTED' | 'PAYMENT_RECEIVED' | 'PAYMENT_APPLIED' | 'BANK_DEPOSIT_POSTED' | 'CARD_PURCHASE_POSTED' | 'CARD_PAYMENT_POSTED' | 'RECONCILIATION_STARTED' | 'RECONCILIATION_COMPLETED'; readonly at: string; readonly detail: Readonly<Record<string, unknown>> }
