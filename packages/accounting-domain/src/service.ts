...
import type { CaseTemplate, JournalEntry, JournalLine, StudentAttempt, TrialBalanceRow } from './model.js';
...
export class PracticeLabService {
  ...
  async instantiate(studentId: string, templateId: string): Promise<StudentAttempt> {
    ...
    const attempt: StudentAttempt = Object.freeze({ id, studentId, templateId, generation: 1, status: 'ACTIVE', entries: [], unlockedDocumentIds: [], accounts: template.accounts.map(a => Object.freeze({ ...a, id: this.system.id(), attemptId: id, sourceTemplateAccountId: a.id })), actions: [{ sequence: 1, kind: 'ATTEMPT_CREATED', at: this.system.now(), detail: {} }] });
