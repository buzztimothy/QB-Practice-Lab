...
import { arControlDifference, customerBalances, reconciliationResult } from './operations.js';
...
const fingerprint = (command: StudentBookkeepingCommand, context: Pick<StudentCommandContext, 'help' | 'note'>) => JSON.stringify({ command, help: context.help, note: context.note });
...
const scenario = (value: P002InstructorState, scenarioId: `SUN-L1-${string}`) => value.provenance.find(item => item.scenarioId === scenarioId);
const assertScenarioTarget = (value: P002InstructorState, targetId: string, scenarioId: `SUN-L1-${string}`) => { if (!scenario(value, scenarioId)?.studentRecordIds.includes(targetId)) throw new InvalidReferenceError('Command unavailable'); };
const scenarioForTarget = (value: P002InstructorState, targetId: string) => value.provenance.find(item => item.studentRecordIds.includes(targetId));
const operationallyLinked = (value: OperationalAttempt, entryId: string) => value.invoices.some(item => item.journalEntryId === entryId) || value.payments.some(item => item.journalEntryId === entryId) || value.deposits.some(item => item.journalEntryId === entryId);
...
function withMutation(session: StudentCommandSession, nextState: OperationalAttempt, input: { readonly action: P002ActionKind; readonly targetId: string; readonly context: StudentCommandContext; readonly before: unknown; readonly after: unknown; readonly hook?: P002CriticalHook; readonly correct?: boolean }): StudentCommandSession {
  ...
  const audited = recordP002Action(updated, input.action, input.targetId, { hook: input.hook, verifiedCorrect: input.correct, selfCorrected: input.correct && wasPreviouslyChanged(oldP002, input.targetId), helpLevel: input.context.help === 'INDEPENDENT' ? undefined : input.context.help, commandId: `${session.attemptId}-command-${revision}`, revision, note: input.context.note, visibleDocumentIds: session.coaching.interaction.evidence.documents.filter(document => document.state === 'AVAILABLE_AT_START' || document.state === 'UNLOCKED').map(document => document.id), before: input.before, after: input.after });
...
function categorize(session: StudentCommandSession, command: Extract<StudentBookkeepingCommand, { type: 'CATEGORIZE' | 'TRANSFER' }>, context: StudentCommandContext): StudentCommandSession {
  const currentState = state(session), current = entry(currentState, command.entryId); if (current.lines.length !== 2 || operationallyLinked(currentState, current.id)) throw new InvalidStateError('Command unavailable');
  ...
  const other = current.lines.find(line => line.id !== cashLine.id)!; const specs = current.lines.map(line => line.id === other.id ? { accountId: target.id, debitCents: line.debitCents, creditCents: line.creditCents } : { accountId: line.attemptAccountId, debitCents: line.debitCents, creditCents: line.creditCents }); const targetScenario = scenarioForTarget(p002(session), current.id)?.scenarioId; const hook = targetScenario === 'SUN-L1-06' && !visibleDocument(session, 'abc-deposit-agreement') ? 'ABC_CLASSIFICATION_GUESSED' : targetScenario === 'SUN-L1-14' && !visibleDocument(session, 'card-clarification-0624') ? 'PERSONAL_CARD_UNSUPPORTED_TREATMENT' : undefined;
  return withMutation(session, replaceEntry(currentState, current.id, specs), { action: 'ACCOUNT_CHANGED', targetId: current.id, context, before: lineSnapshot(current), after: specs, hook });
}
...
function match(session: StudentCommandSession, command: Extract<StudentBookkeepingCommand, { type: 'MATCH' }>, context: StudentCommandContext): StudentCommandSession {
  ...
  if (payment?.reference === 'RCPT-MARTINEZ-0612' && bank.source?.kind === 'P002_ADDED_BANK_INCOME') {
    assertScenarioTarget(p002(session), bank.id, 'SUN-L1-07');
  ...
  } else if (targetEntry && bank.source?.kind === 'P002_ADDED_NOT_MATCHED' && targetEntry.id !== bank.id && targetEntry.occurredOn === bank.occurredOn && lineSnapshot(targetEntry).reduce((sum, line) => sum + line.debitCents, 0) === lineSnapshot(bank).reduce((sum, line) => sum + line.debitCents, 0)) { const provenance = scenario(p002(session), 'SUN-L1-11'); if (!provenance?.studentRecordIds.includes(bank.id) || !provenance.cleanRecordIds.includes(targetEntry.id)) throw new InvalidReferenceError('Command unavailable'); currentState = removeEntry(currentState, bank.id); }
...
function correctNamed(session: StudentCommandSession, command: Exclude<StudentBookkeepingCommand, { type: 'REVIEW' | 'VERIFY_UNCHANGED' | 'FLAG_UNRESOLVED' | 'MATCH' | 'CATEGORIZE' | 'TRANSFER' | 'EXCLUDE' | 'VOID' | 'REAPPLY_PAYMENT' | 'CORRECT_PAYROLL' | 'CONSOLIDATE_ACCOUNTS' | 'SET_RECONCILIATION_LINE' | 'FINISH_RECONCILIATION' }>, context: StudentCommandContext): StudentCommandSession {
  const currentState = state(session), current = entry(currentState, command.entryId); let specs: readonly { accountId: string; debitCents?: number; creditCents?: number }[], resolutionHook: P002CriticalHook | undefined;
  if (command.type === 'CORRECT_OWNER_CONTRIBUTION') { assertScenarioTarget(p002(session), current.id, 'SUN-L1-03'); specs = [{ accountId: accountNamed(currentState, 'Operating Checking').id, debitCents: 500000 }, { accountId: accountNamed(currentState, 'Owner Contributions').id, creditCents: 500000 }]; }
  else if (command.type === 'CORRECT_PRESSURE_WASHER') { assertScenarioTarget(p002(session), current.id, 'SUN-L1-05'); specs = [{ accountId: accountNamed(currentState, 'Tools & Equipment').id, debitCents: 680000 }, { accountId: accountNamed(currentState, 'Operating Checking').id, creditCents: 680000 }]; }
  else if (command.type === 'CORRECT_VEHICLE_LOAN') { assertScenarioTarget(p002(session), current.id, 'SUN-L1-04'); if (!visibleDocument(session, 'vehicle-loan-june')) throw new InvalidReferenceError('Command unavailable'); specs = [{ accountId: accountNamed(currentState, 'Vehicle Loan Payable').id, debitCents: 69000 }, { accountId: accountNamed(currentState, 'Interest Expense').id, debitCents: 23500 }, { accountId: accountNamed(currentState, 'Operating Checking').id, creditCents: 92500 }]; }
  else if (command.type === 'RESOLVE_ABC') { assertScenarioTarget(p002(session), current.id, 'SUN-L1-06'); if (!visibleDocument(session, 'abc-deposit-agreement')) throw new InvalidReferenceError('Command unavailable'); specs = [{ accountId: accountNamed(currentState, 'Trailer Deposit').id, debitCents: 240000 }, { accountId: accountNamed(currentState, 'Operating Checking').id, creditCents: 240000 }]; if (p002(session).attempt.auditTrail.some(event => event.hook === 'ABC_CLASSIFICATION_GUESSED')) resolutionHook = 'ABC_CLASSIFICATION_GUESSED'; }
  else if (command.type === 'RESOLVE_PERSONAL_CARD') { assertScenarioTarget(p002(session), current.id, 'SUN-L1-14'); if (!visibleDocument(session, 'card-clarification-0624')) throw new InvalidReferenceError('Command unavailable'); specs = [{ accountId: accountNamed(currentState, 'Owner Draws').id, debitCents: 28643 }, { accountId: accountNamed(currentState, 'Gulf Coast Business Visa').id, creditCents: 28643 }]; if (p002(session).attempt.auditTrail.some(event => event.hook === 'PERSONAL_CARD_UNSUPPORTED_TREATMENT')) resolutionHook = 'PERSONAL_CARD_UNSUPPORTED_TREATMENT'; }
  else if (command.type === 'CORRECT_CARD_PAYMENT') { assertScenarioTarget(p002(session), current.id, 'SUN-L1-15'); specs = [{ accountId: accountNamed(currentState, 'Gulf Coast Business Visa').id, debitCents: 350000 }, { accountId: accountNamed(currentState, 'Operating Checking').id, creditCents: 350000 }]; }
  else if (command.type === 'CORRECT_DEPOSIT_TRANSFER') { assertScenarioTarget(p002(session), current.id, 'SUN-L1-12'); const amount = current.lines.reduce((sum, line) => sum + line.debitCents, 0); specs = [{ accountId: accountNamed(currentState, 'Operating Checking').id, debitCents: amount }, { accountId: accountNamed(currentState, 'Undeposited Funds').id, creditCents: amount }]; }
  else { assertScenarioTarget(p002(session), current.id, 'SUN-L1-13'); const completed = currentState.reconciliations.filter(item => item.status === 'COMPLETED' && item.clearedJournalLineIds.some(id => current.lines.some(line => line.id === id))); const prior = new Map<string, string>(); for (const reconciliation of completed) for (const line of current.lines) if (reconciliation.clearedLineFingerprints[line.id]) prior.set(line.id, reconciliation.clearedLineFingerprints[line.id]); if (prior.size === 0) throw new InvalidReferenceError('Command unavailable'); const known = [...prior.values()].map(value => value.split(':')).reduce((sum, [debit, credit]) => sum + Number(debit) + Number(credit), 0); specs = current.lines.map(line => { const stored = prior.get(line.id); if (stored) { const [debit, credit, accountId] = stored.split(':'); return { accountId, debitCents: Number(debit), creditCents: Number(credit) }; } return line.debitCents > 0 ? { accountId: line.attemptAccountId, debitCents: known } : { accountId: line.attemptAccountId, creditCents: known }; }); }
  let next = replaceEntry(currentState, current.id, specs); for (const spec of specs) { const debitCents = spec.debitCents ?? 0, creditCents = spec.creditCents ?? 0; next = replaceLineId(next, current.id, line => line.attemptAccountId === spec.accountId && line.debitCents === debitCents && line.creditCents === creditCents, missingHistoricalLineId(next, spec.accountId, debitCents, creditCents)); } return withMutation(session, next, { action: 'TRANSACTION_CORRECTED', targetId: current.id, context, before: lineSnapshot(current), after: specs, hook: resolutionHook, correct: true });
}
...
function payroll(session: StudentCommandSession, command: Extract<StudentBookkeepingCommand, { type: 'CORRECT_PAYROLL' }>, context: StudentCommandContext): StudentCommandSession {
  if (!visibleDocument(session, 'payroll-report-june-14') || command.entryIds.length !== 3 || new Set(command.entryIds).size !== 3) throw new InvalidReferenceError('Command unavailable'); let next = state(session); const requested = command.entryIds.map(id => entry(next, id)); if (requested.some(item => item.source?.kind !== 'P002_SIMPLIFIED_PAYROLL')) throw new InvalidReferenceError('Command unavailable'); for (const item of requested) assertScenarioTarget(p002(session), item.id, 'SUN-L1-16'); const checkingId = accountNamed(next, 'Operating Checking').id; const byWithdrawal = new Map(requested.map(item => [item.lines.find(line => line.attemptAccountId === checkingId)?.creditCents, item] as const)); const originals = [byWithdrawal.get(800030), byWithdrawal.get(269970), byWithdrawal.get(10000)]; if (originals.some(item => !item)) throw new InvalidReferenceError('Command unavailable');
  const checking = checkingId, liability = accountNamed(next, 'Payroll Liabilities').id;
  ...
  return withMutation(session, next, { action: 'TRANSACTION_CORRECTED', targetId: originals[0]!.id, context, before: originals.map(item => lineSnapshot(item!)), after: originals.map(item => lineSnapshot(entry(next, item!.id))), correct: true });
}
...
function reapply(session: StudentCommandSession, command: Extract<StudentBookkeepingCommand, { type: 'REAPPLY_PAYMENT' }>, context: StudentCommandContext): StudentCommandSession {
  const currentState = state(session), payment = currentState.payments.find(item => item.id === command.paymentId), from = currentState.invoices.find(item => item.id === command.fromInvoiceId), to = currentState.invoices.find(item => item.id === command.toInvoiceId); if (!payment || !from || !to || payment.customerId !== from.customerId || payment.customerId !== to.customerId || payment.reference !== 'RCPT-REYNOLDS-0615' || from.number !== 'REY-B' || to.number !== 'REY-A') throw new InvalidReferenceError('Command unavailable');
  const applications = currentState.applications.filter(item => item.paymentId === payment.id), moving = applications.find(item => item.invoiceId === from.id); if (!moving) throw new InvalidStateError('Command unavailable'); assertScenarioTarget(p002(session), moving.id, 'SUN-L1-08'); const targetOpenCents = to.lineItems.reduce((sum, item) => sum + item.amountCents, 0) - currentState.applications.filter(item => item.invoiceId === to.id).reduce((sum, item) => sum + item.amountCents, 0); if (moving.amountCents > targetOpenCents) throw new InvalidStateError('Command unavailable');
  ...
  const next = { ...currentState, applications: nextApplications, invoices: currentState.invoices.map(item => item.id === from.id || item.id === to.id ? { ...item, status: invoiceOpen(item.id) === 0 ? 'PAID' as const : 'OPEN' as const } : item) }; const arAccount = accountNamed(next, 'Accounts Receivable'); if (arControlDifference(next, arAccount.id) !== 0 || customerBalances(next).some(item => item.invoiceOpenCents < 0)) throw new InvalidStateError('Command unavailable');
...
export function executeStudentCommand(session: StudentCommandSession, command: StudentBookkeepingCommand, context: StudentCommandContext): StudentCommandSession {
  if (!context.idempotencyKey || context.idempotencyKey.length > 100 || (context.note?.length ?? 0) > 1000) throw new InvalidReferenceError('Command unavailable'); const inputFingerprint = fingerprint(command, context), receipt = session.receipts.find(item => item.idempotencyKey === context.idempotencyKey); if (receipt) { if (receipt.fingerprint !== inputFingerprint) throw new InvalidStateError('Command conflict'); return session; }
  ...
  else if (command.type === 'EXCLUDE' || command.type === 'VOID') { const currentState = state(session), current = entry(currentState, command.entryId); if (operationallyLinked(currentState, current.id)) throw new InvalidReferenceError('Command unavailable'); const hook = protectedHook(p002(session), current.id), provenance = scenarioForTarget(p002(session), current.id), correct = !!provenance && !provenance.cleanRecordIds.includes(current.id) && ['SUN-L1-01', 'SUN-L1-07', 'SUN-L1-11'].includes(provenance.scenarioId); result = withMutation(session, removeEntry(currentState, current.id), { action: 'TRANSACTION_VOIDED', targetId: current.id, context, before: lineSnapshot(current), after: { excluded: command.type === 'EXCLUDE' }, hook, correct }); if (command.type === 'EXCLUDE') result = deepFreeze({ ...result, excludedBankActivities: [...session.excludedBankActivities, current] }); }
...
export function studentCommandView(value: StudentCommandSession) { const bookkeeping = p002StudentView(p002(value)), personalAuthorized = visibleDocument(value, 'card-clarification-0624'); const safeBookkeeping = personalAuthorized ? bookkeeping : { ...bookkeeping, attempt: { ...bookkeeping.attempt, accounts: bookkeeping.attempt.accounts.filter(item => item.name !== 'Owner Draws') } }; return deepFreeze({ attemptNumber: value.coaching.generation, revision: value.revision, bookkeeping: safeBookkeeping, excludedBankActivities: value.excludedBankActivities.map(item => ({ id: item.id, occurredOn: item.occurredOn, description: 'Excluded bank activity', status: 'EXCLUDED' as const })), coaching: coachingStudentView(value.coaching), commands: value.receipts.map(item => ({ commandId: item.commandId, revision: item.revision })) }); }

export interface StudentCommandStore { findForStudent(attemptId: string, studentId: string): Promise<StudentCommandSession | null>; save(value: StudentCommandSession, expectedRevision: number): Promise<boolean> }
export class StudentBookkeepingCommandService {
  ...
  async execute(studentId: string, attemptId: string, command: StudentBookkeepingCommand, context: StudentCommandContext) { const current = await this.owned(studentId, attemptId), updated = executeStudentCommand(current, command, context); if (await this.store.save(updated, current.revision)) return studentCommandView(updated); const latest = await this.owned(studentId, attemptId); return studentCommandView(executeStudentCommand(latest, command, context)); }
