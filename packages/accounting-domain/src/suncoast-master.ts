...
export async function buildSuncoastMasterCase():Promise<SuncoastMasterCase>{
  ...
  const registerBalance=(accountId:string,through:string,liability=false)=>store.state.attempt.entries.filter(entry=>entry.occurredOn<=through).flatMap(entry=>entry.lines).filter(line=>line.attemptAccountId===accountId).reduce((sum,line)=>sum+(liability?line.creditCents-line.debitCents:line.debitCents-line.creditCents),0);
  const reconcile=async(accountId:string,liability:boolean):Promise<{reconciliation:Reconciliation;truth:SuncoastStatementTruth}>=>{const juneLines=store.state.attempt.entries.filter(entry=>entry.occurredOn>='2026-06-01'&&entry.occurredOn<='2026-06-30').flatMap(entry=>entry.lines).filter(line=>line.attemptAccountId===accountId);const beginning=registerBalance(accountId,'2026-05-31',liability);const ending=registerBalance(accountId,'2026-06-30',liability);const reconciliation=await service.startReconciliation('instructor-master',SUNCOAST_ATTEMPT_ID,{accountId,beginningDate:'2026-06-01',endingDate:'2026-06-30',beginningBalanceCents:beginning,endingBalanceCents:ending,clearedJournalLineIds:juneLines.map(line=>line.id)});await service.completeReconciliation('instructor-master',SUNCOAST_ATTEMPT_ID,reconciliation.id);const result=reconciliationResult(store.state,reconciliation);return{reconciliation,truth:{accountId,beginningBalanceCents:beginning,endingBalanceCents:ending,clearedLineIds:juneLines.map(line=>line.id),reconciliationId:reconciliation.id,differenceCents:result.differenceCents}}};
  const checking=await reconcile(id('Operating Checking'),false);const card=await reconcile(id('Gulf Coast Business Visa'),true);
