...
describe('D-002 production authentication boundary',()=>{
  ...
  it('keeps destructive test and Preview deployment database targets mutually exclusive',()=>{
    ...
    expect(()=>assertPreviewDeployDatabase('postgresql://bbb_preview_deploy_lp:secret@ep-direct.example/bbb_practice_preview?sslmode=require','bbb_practice_preview','ep-direct.example')).not.toThrow();
    for(const value of ['postgresql://bbb_preview_deploy_lp:secret@ep-direct.example/bbb_practice_preview_validation?sslmode=require','postgresql://bbb_preview_deploy_lp:secret@ep-direct-pooler.example/bbb_practice_preview?sslmode=require','postgresql://other:secret@ep-direct.example/bbb_practice_preview?sslmode=require','postgresql://bbb_preview_deploy_lp:secret@ep-other.example/bbb_practice_preview?sslmode=require',' postgresql://bbb_preview_deploy_lp:secret@ep-direct.example/bbb_practice_preview?sslmode=require'])expect(()=>assertPreviewDeployDatabase(value,'bbb_practice_preview','ep-direct.example')).toThrow();
    expect(()=>assertPreviewDeployDatabase('postgresql://bbb_preview_deploy_lp:secret@ep-direct.example/bbb%5Fpractice%5Fpreview?sslmode=require','bbb_practice_preview','ep-direct.example')).not.toThrow();
