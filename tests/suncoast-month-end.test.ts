...
function answerAll(meeting: ReturnType<typeof submitMonthEndExplanation>) { let value=meeting; for(const followUp of meeting.explanationEvidence!.followUps) value=answerMonthEndFollowUp(value,followUp.id,followUp.kind==='PROFIT_WITHDRAWAL'?'Profit is not the same as available cash; we should consider obligations and distribution guidance.':followUp.kind==='RECEIVABLE_CONCERN'?'The books show outstanding balances, so a collection follow-up is appropriate without assuming they are uncollectible.':'The books show the balances, but they do not establish whether the debt is a problem without payment and business context.'); return value; }
...
describe('P-007 final meeting entry and financial truth', () => {
  it('blocks entry without an authorized ready close and hides the reason', async () => {
    ...
    const postCloseAction={...p002,attempt:{...p002.attempt,auditTrail:recordP002Action(p002,'TRANSACTION_REVIEWED','post-close-review').attempt.auditTrail}};
    await expect(beginMonthEndMeeting(assessment,postCloseAction)).rejects.toBeInstanceOf(InvalidStateError);
  });
...
describe('P-007 explanation evidence and assessment', () => {
  ...
  it.each([
    ...
    ['rounded trend','June revenue was about 28% higher than April, rising to $43,000.',10],
    ['minor rounding','June revenue was approximately $43,001, up from April.',8],
  ...
  it('changes Month-End from NOT_ASSESSED to assessed 100-point evaluation without mutating history',async()=>{const{assessment,p002}=await fixture();expect(assessment.snapshots[0].competencies.at(-1)?.status).toBe('NOT_ASSESSED');const historical=assessment.snapshots[0];const meeting=answerAll(submitMonthEndExplanation(await beginMonthEndMeeting(assessment,p002),strong));const result=applyMonthEndAssessment(assessment,meeting,complete);expect(result.assessment.snapshots[0]).toEqual(historical);expect(result.assessment.snapshots.at(-1)).toMatchObject({pointsEarned:100,pointsAssessed:100,classification:'CLIENT_READY'});expect(result.assessment.snapshots.at(-1)?.competencies.at(-1)).toMatchObject({status:'ASSESSED',earnedPoints:10,availablePoints:10});expect(result.meeting.status).toBe('ASSESSED')});

  it('keeps readiness and critical overrides authoritative after all 100 points are assessed',async()=>{const{assessment,p002}=await fixture();const meeting=answerAll(submitMonthEndExplanation(await beginMonthEndMeeting(assessment,p002),strong));const unresolved={...complete,checking:false,reconciled:false,complete:false};expect(applyMonthEndAssessment(assessment,meeting,unresolved).assessment.snapshots.at(-1)?.classification).toBe('RETURN_TO_LAB');const critical=appendAssessmentEvidence(assessment,{competency:'PROFESSIONAL_JUDGMENT',type:'CRITICAL_EVENT',source:{kind:'P002_ACTION',id:'critical',attemptId:assessment.attemptId},scenarioId:'SUN-L1-20',criticalHook:'RECONCILIATION_ADJUSTMENT_FORCED',severity:'CRITICAL',outcome:'INAPPROPRIATE',selfCorrected:false,resolved:false,instructorExplanation:'Unresolved critical event.'});expect(applyMonthEndAssessment(critical,meeting,complete).assessment.snapshots.at(-1)?.classification).toBe('RETURN_TO_LAB')});

  it('assesses follow-up professional boundaries without replacing the independent explanation',async()=>{const{assessment,p002}=await fixture();let meeting=submitMonthEndExplanation(await beginMonthEndMeeting(assessment,p002),strong);for(const followUp of [...meeting.explanationEvidence!.followUps]) meeting=answerMonthEndFollowUp(meeting,followUp.id,'You can withdraw all of it, and I guarantee the receivables will be collected.');const applied=applyMonthEndAssessment(assessment,meeting,complete);expect(applied.assessment.snapshots.at(-1)?.competencies.at(-1)?.earnedPoints).toBe(9);expect(applied.meeting.explanationEvidence?.explanation).toBe(strong);expect(applied.meeting.explanationEvidence?.helpState).toBe('INDEPENDENT')});

  it('records append-only meeting, context, explanation, follow-up, help, and assessment audit chronology',async()=>{const{assessment,p002}=await fixture();let meeting=submitMonthEndExplanation(await beginMonthEndMeeting(assessment,p002),strong);meeting=answerAll(meeting);meeting=requestPostExplanationHelp(meeting,'HINT');const applied=applyMonthEndAssessment(assessment,meeting,complete);expect(applied.meeting.audit.map(item=>item.kind)).toEqual(['MEETING_OPENED','FINANCIAL_CONTEXT_SNAPSHOTTED','EXPLANATION_SUBMITTED','FOLLOW_UP_TRIGGERED','FOLLOW_UP_ASKED','FOLLOW_UP_TRIGGERED','FOLLOW_UP_ASKED','FOLLOW_UP_RESPONSE_RECORDED','FOLLOW_UP_RESPONSE_RECORDED','HELP_RECORDED','ASSESSMENT_EVIDENCE_CREATED','ASSESSMENT_SNAPSHOT_CREATED']);expect(applied.meeting.audit.map(item=>item.sequence)).toEqual(applied.meeting.audit.map((_item,index)=>index+1));expect(Object.isFrozen(applied.meeting.audit)).toBe(true)});
});
...
describe('P-007 secrecy, isolation and reset',()=>{
  ...
  it('rejects cross-attempt context and evidence integration',async()=>{const left=await fixture('left'),right=await fixture('right');await expect(beginMonthEndMeeting(left.assessment,right.p002)).rejects.toBeInstanceOf(InvalidReferenceError);const meeting=answerAll(submitMonthEndExplanation(await beginMonthEndMeeting(left.assessment,left.p002),strong));expect(()=>applyMonthEndAssessment(right.assessment,meeting,complete)).toThrow(InvalidReferenceError)});
